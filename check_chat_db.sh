...
echo -e "_________CHAT_APP_USER_BLOCKED_USERS_________\n" >> chat_models.txt
docker exec -e PGPASSWORD=$CHAT_DB_PASSWORD chat psql -U $CHAT_DB_USER -d $CHAT_DB_NAME -h $CHAT_DB_HOST -c "SELECT * FROM chat_app_user_blocked_users;" >> chat_models.txt
