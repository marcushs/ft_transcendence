...
class ChangeUserInfosView(View):
    ...
    def change_username(self, User, request):
        ...
        try:
            send_request(request_type='POST',request=request, url='http://twofactor:8000/api/twofactor/update_user/', payload=payload)
            send_request(request_type='POST',request=request, url='http://auth:8000/api/auth/update_user/', payload=payload)
            send_request(request_type='POST',request=request, url='http://friends:8000/api/friends/update_user/', payload=payload)
        except Exception:
            pass
        request.user.save()
